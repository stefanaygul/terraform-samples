...
locals {
  execute_command_configuration = {
    logging = "OVERRIDE"
    log_configuration = {
      cloud_watch_log_group_name = try(aws_cloudwatch_log_group.entr_cw_log_group[0].name, null)
    }
  }
}

resource "aws_ecs_cluster" "entr_cluster" {
...
resource "aws_cloudwatch_log_group" "entr_cw_log_group" {
...
resource "aws_ecs_cluster_capacity_providers" "entr_cluster_capacity_provider" {
  count = var.create && length(merge(var.fargate_capacity_providers, var.autoscaling_capacity_providers)) > 0 ? 1 : 0

  cluster_name = aws_ecs_cluster.entr_cluster[0].name
