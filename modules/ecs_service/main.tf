...
locals {
  ...
  network_configuration = {
    ...
    security_groups  = flatten(concat([try(aws_security_group.entr_ecs_sg[0].id, [])], var.security_group_ids))
    subnets          = var.subnet_ids
  }
}

resource "aws_ecs_service" "entr_service" {
...
locals {
  ...
  max_task_def_revision = local.create_task_definition ? max(aws_ecs_task_definition.entr_task_def[0].revision, data.aws_ecs_task_definition.entr_data_task_def[0].revision) : 0
  task_definition       = local.create_task_definition ? "${aws_ecs_task_definition.entr_task_def[0].family}:${local.max_task_def_revision}" : var.task_definition_arn
}

# This allows us to query both the existing as well as Terraform's state and get
# and get the max version of either source, useful for when external resources
# update the container definition
data "aws_ecs_task_definition" "entr_data_task_def" {
  count = local.create_task_definition ? 1 : 0

  task_definition = aws_ecs_task_definition.entr_task_def[0].family

  depends_on = [
    # Needs to exist first on first deployment
    aws_ecs_task_definition.entr_task_def
  ]
}

resource "aws_ecs_task_definition" "entr_task_def" {
  count = local.create_task_definition ? 1 : 0
...
resource "aws_ecs_task_set" "entr_task_set" {
  ...
  service         = try(aws_ecs_service.entr_service[0].id, aws_ecs_service.ignore_task_definition[0].id)
  cluster         = var.cluster_arn
...
resource "aws_appautoscaling_target" "entr_app_asg" {
  ...
  resource_id        = "service/${local.cluster_name}/${try(aws_ecs_service.entr_service[0].name, aws_ecs_service.ignore_task_definition[0].name)}"
  scalable_dimension = "ecs:service:DesiredCount"
  service_namespace  = "ecs"
}

resource "aws_appautoscaling_policy" "entr_app_asg_policy" {
  ...
  resource_id        = aws_appautoscaling_target.entr_app_asg[0].resource_id
  scalable_dimension = aws_appautoscaling_target.entr_app_asg[0].scalable_dimension
  service_namespace  = aws_appautoscaling_target.entr_app_asg[0].service_namespace
...
resource "aws_appautoscaling_scheduled_action" "entr_app_asg_schedule" {
...
data "aws_subnet" "entr_ecs_subnet" {
  count = local.create_security_group ? 1 : 0

  id = element(var.subnet_ids, 0)
}

resource "aws_security_group" "entr_ecs_sg" {
  count = local.create_security_group ? 1 : 0

  name        = var.security_group_use_name_prefix ? null : local.security_group_name
  name_prefix = var.security_group_use_name_prefix ? "${local.security_group_name}-" : null
  description = var.security_group_description
  vpc_id      = data.aws_subnet.entr_ecs_subnet[0].vpc_id
...
resource "aws_security_group_rule" "entr_ecs_asg_rule" {
  for_each = { for k, v in var.security_group_rules : k => v if local.create_security_group }

  # Required
  security_group_id = aws_security_group.entr_ecs_sg[0].id
