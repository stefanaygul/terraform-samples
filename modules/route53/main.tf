...
resource "aws_route53_record" "record" {
  ...
  name                             = each.value.name != "" ? (lookup(each.value, "full_name_override", false) ? each.value.name : "${each.value.name}.${data.aws_route53_zone.zone[0].name}") : data.aws_route53_zone.zone[0].name
  ...
  dynamic "alias" {
    ...
    content {
      ...
      zone_id                = try(each.value.alias.zone_id, data.aws_route53_zone.zone[0].zone_id)
